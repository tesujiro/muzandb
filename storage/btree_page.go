...
func (btree *Btree) ToPageDataHeader(node *BtreeNode) *PageData {
	...
	if node.ParentPage != nil {
		header[i] = byte(node.ParentPage.file.FID)
		i += 1
		endian.PutUint32(header[i:], node.ParentPage.pagenum)
		i += 4
	} else {
		i += 5
	}
	...
	// Header: NextLeafNode
	if node.Leaf {
		header[i] = byte(node.NextLeafPage.file.FID)
		i += 1
		endian.PutUint32(header[i:], node.NextLeafPage.pagenum)
		i += 4
	}
...
func (btree *Btree) ToNode(pd *PageData) (*BtreeNode, error) {
	...
	if node.Leaf {
		nextPage := &Page{}
		node.NextLeafPage = nextPage

		node.NextLeafPage.file.FID = FID(uint8(data[i]))
		i += 1
		node.NextLeafPage.pagenum = endian.Uint32(data[i:])
		i += 4
	}
